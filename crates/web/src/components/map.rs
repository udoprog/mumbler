...
use api::{
    Color, Extent, Key, LocalUpdateBody, Pan, PeerId, PublicKey, RemoteId, RemoteUpdateBody,
...
impl Map {
                ...
                let update = {
                    ...
                    match body {
                        LocalUpdateBody::ObjectCreated { object } => 'done: {
                            let Some(object) = LocalObject::from_remote(PeerId::ZERO, &object)
                            else {
                                break 'done false;
                            };
            ...
            Msg::RemoteUpdate(body) => {
                ...
                let update = match body {
                    ...
                    RemoteUpdateBody::PeerLeave { peer_id } => {
                        ...
                        order.retain(|peer_id| peer_id != peer_id);
                        true
                    }
    ...
    fn start_translate(&mut self, object_id: RemoteId) {
    ...
    fn drag_end(&mut self, ctx: &Context<Self>, object_id: RemoteId) -> Result<bool, Error> {
        ...
        if group_changed {
            self._set_group =
                self::object_update(ctx, object_id, Key::GROUP, Value::from(new_group.id));
        }
